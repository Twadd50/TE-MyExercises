...
WHERE district IN ('Pennsylvania', 'West Virginia', 'Kentucky', 'Indiana', and 'Michigan')
...
-- (smallest_country_population in world: 50)
      ?          SELECT MIN(population) AS smallest_population FROM country
...
-- (largest continental surface area: 31881000, "Asia")
                 SELECT MAX(surfacearea), continent FROM ORDER BY surfacearea DESC
...
-- (highest life expectancies in world: 83.5, 0.006, "Andorra")
                 SELECT(population / surfacearea) * lifeexpectancy FROM country ORDER BY lifeexpectancy DESC
...
-- (45 rows)
                SELECT id FROM city WHERE countrycode = 'USA'
...
SELECT name, MAX(population) country FROM city ORDER BY city DESC
